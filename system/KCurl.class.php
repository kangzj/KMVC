...
class KCurl {
    ...
    //	private $headers = array ('User-Agent:Mozilla/5.0 (Windows NT 6.1) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.11 Safari/535.19'); // Populates extra HTTP headers
    public $error_code; // Error code returned as an int
    public $error_string; // Error message returned as a string
    public $info; // Returned after request (elapsed time, etc)

    function __construct($url = '') {
        //transfer user's headers to inner systems
        $headers = array();
        $headers [] = 'Connection: Keep-Alive';
        $this->headers = $this->makeHeader($headers);
        //--end
    ...
    private function makeHeader($headers) {
        $newHeaders = array();
        foreach ($headers as $k => $v) {
            $newHeaders [] = trim($k) . ':' . trim($v);
        }
        return $newHeaders;
    }
...
//        }
    ...
    public function execute() {
        ...
        if (!isset($this->options [CURLOPT_FAILONERROR])) {
            $this->options [CURLOPT_FAILONERROR] = 1;
        }
