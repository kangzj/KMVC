...
class KAutoload {
    ...
    public static function init($dir, $ext) {
        ...
        spl_autoload_register(array('KAutoload', "loadClass"));
    }
    ...
    public static function glob_recursive($dir, $ext) {
        $files = glob($dir . $ext);
        foreach (glob($dir . '*', GLOB_ONLYDIR | GLOB_NOSORT | GLOB_MARK) as $subdir) {
            $files = array_merge($files, self::glob_recursive($subdir . '/', $ext));
        }
        return $files;
    }
    ...
    public static function loadClass($className) {
        ...
        if (isset(self::$classFileMapping [$className]) && file_exists(self::$classFileMapping [$className])) {
            ...
            if (isset(self::$classFileMapping [$className]) && file_exists(self::$classFileMapping [$className])) {
                ...
                throw new KException('autoload error: class ' . $className . ' not found');
            }
        }
    }
