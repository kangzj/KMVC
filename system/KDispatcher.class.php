...
class KDispatcher {
    ...
    public function dispatch() {
        ...
        //call the action
        call_user_func_array(array($this->controller, $this->action_name), $action_params);
        if (KMVC_TIMER) {
            /* insert timing info */
    ...
    private function setupController() {
        $this->controller_name = !empty($this->url_segments [1]) ? preg_replace('!\W!', '', $this->url_segments [1]) : KConfig::defaultController;
        $this->controller_name = $this->controller_name;
        $this->controller = new $this->controller_name($this);
    }

    private function setupAction() {
        $this->action_name = !empty($this->url_segments [2]) ? preg_replace('!\W!', '', $this->url_segments [2]) : KConfig::defaultAction;
    }
